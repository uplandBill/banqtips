...
$eeList = "Select distinct f1.emplid, n1.name from events e1, funcEmps f1, employees n1 where e1.unitId = '$unitId' and e1.wkendDate = '$wkendDate' and f1.eventNum = e1.eventNum and f1.emplid = n1.emplid order by n1.name";
$eeRes  = mysql_query($eeList) or die('Employee list data failed: ' . mysql_error());
while ($eeRec = mysql_fetch_array($eeRes, MYSQL_NUM)) {
   ...
   $eventSum = $eventSum . " from events e, funcTypes t, funcEmps f, rooms r where e.unitId = '$unitId' and e.wkendDate = '$wkendDate' and t.funcType = e.funcType and t.unitId = e.unitId and f.eventNum = e.eventNum";
   $eventSum = $eventSum . " and f.emplid = '$emplid' and r.unitId = e.unitId and r.roomNum = e.roomNum order by e.funcDate, e.eventNum";

   $sqlRes  = mysql_query($eventSum) or die('ee summ SQL data failed: ' . mysql_error());
   $lineCnt = 0;
   $printCnt = 0;
   $pageCnt = 0;
