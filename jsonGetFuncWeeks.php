<?php
  ...
  if ($userId == "") {
    ...
    if ($unitId == "") {
       ...
       $getSQL = $getSQL ." Where u.unitId='$unitId' and c.unitId=u.unitId and c.weekStatus = 'C'";
       $getRes = mysql_query($getSQL) or die('select weekend data failed: ' . mysql_error());
       while ($recs = mysql_fetch_array($getRes, MYSQL_NUM)) {
          $wkendDate = $recs[1];
          $wkEndForm = $recs[2];
          $unitName  = $recs[3];
          $csvFormat = $recs[4];
